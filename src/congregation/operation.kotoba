(ns congregation.operation
  "OperationActor -- one congregation operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (CongregationOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Congregational Governance
  ...
  One graph run = one congregation operation (intake -> advise ->
  ...
  decision to a human operator (clergy/religious leadership). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/finalize-pastoral-referral`/`:actuation/
  publish-doctrinal-statement` ALWAYS reach this node when the
  governor is clean -- see `congregation.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [congregation.congregationadvisor :as congregationadvisor]
...
(defn build
  ...
  `congregation.store/Store`).
  opts:
    :advisor      -- a `congregation.congregationadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (congregationadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CongregationOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (congregationadvisor/-advise advisor store request)]
            {:proposal p :audit [(congregationadvisor/trace request p)]})))

      ;; Congregational Governance Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
